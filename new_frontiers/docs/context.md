...
When two color singlets bind (e.g., nucleosynthesis), they fuse into a single unified geometric boundary ($\mathrm{Len}_{\mathrm{new\_virtual}} = 1$). This strict topological counting rule ensures the strong residual force calculates exactly, preventing factor-of-2 scaling errors in the nuclear mass defect.
...
$$g_{\pi NN} = \frac{g_A m_N}{f_\pi}$$
   ...
   It then applies the continuous Yukawa integral to calculate the exact negative mass defect scale ($\kappa_{\mathrm{residual}}$) for nuclear synthesis:
